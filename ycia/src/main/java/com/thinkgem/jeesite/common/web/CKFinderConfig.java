...
//			this.baseURL = ServletContextFactory.getServletContext().getContextPath()+"/userfiles/"+principal+"/";
			...
			this.baseURL = FileUtils.path(Servlets.getRequest().getContextPath().substring(0,Servlets.getRequest().getContextPath().lastIndexOf("/antu-ydyy")) + Global.USERFILES_BASE_URL + principal + "/");
			this.baseDir = FileUtils.path(Global.getUserfilesBaseDir().substring(0,Global.getUserfilesBaseDir().lastIndexOf("/antu-ydyy")) + Global.USERFILES_BASE_URL + principal + "/");
			System.out.println(Servlets.getRequest().getContextPath()+"==="+Global.getUserfilesBaseDir()+"==="+this.baseURL+"===="+this.baseDir);
		} catch (Exception e) {
