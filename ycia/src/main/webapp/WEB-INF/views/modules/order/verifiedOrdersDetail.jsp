						...
						<tr>
							...
							<td>
								...
								<c:if test="${list.sealCret!=''}">
									<img src="${list.sealCret}" width="200" height="200">
								</c:if>
							</td>
							<td>
								<input type="hidden" id="payCret${ord.index}" name="payCret${ord.index}"/>
								<c:if test="${list.payCret!=''}">
									<img src="${list.payCret}" width="200" height="200">
								</c:if>
							</td>
							<c:if test="${list.status=='40'}">
								<td>
									<input id="pass" class="btn btn-primary" type="button" value="审核通过" />
									<input id="noPass" class="btn btn-primary" type="button" value="审核不通过" />
								</td>
							</c:if>
						</tr>
					</c:if>
	...
	<script type="text/javascript">
			...
			$("#btnSubmit").bind("click",function(){
				$('#contentTable').find('tr').each(function(){
					...
					if(sealCret!== undefined && sealCret!== '' && payCret!==undefined && payCret!==''){
						if(sealCret!='' && payCret!=''){
							var orderCompanyId =  $(this).find('td:eq(4) input:eq(0)').val(); 
							var companyId =  $(this).find('td:eq(4) input:eq(1)').val(); 
							
							window.location = "${ctx}/order/orders/uploadCret?orderCompanyId="+orderCompanyId+"&companyId="+companyId+"&sealCret="+sealCret+"&payCret="+payCret;
						}else if(sealCret=== '' || payCret===''){
							alert("盖章凭证及支付凭证必须同时上传");					
						}
					}
		        })
	       	});
		...
		$("#pass").bind("click",function(){
			$('#contentTable').find('tr').each(function(){
				...
				window.location = "${ctx}/order/orders/updateStatus?orderCompanyId="+orderCompanyId+"&companyId="+companyId+"&orderNo="+orderNo;
			})
        });
       	
       	$("#noPass").bind("click",function(){
       		window.location = "${ctx}/order/orders/verifiedlist";
   		});
